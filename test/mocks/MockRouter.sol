...
contract MockRouter is IRouterClientExtended {
    ...
    mapping(uint256 => bool) internal s_supportedChains;
    ...
    function switchChain(uint64 chainSelector) external {
       s_currentChainSelector = chainSelector;
    }

    function supportChain(uint256 chainSelector) external {
        s_supportedChains[chainSelector] = true;
    }

    function getCurrentChainSelector() external view returns (uint256) {
        return s_currentChainSelector;
    }

    function deployOnRamp() public {
        address where = getOnRamp(0);
        ...
        returns (bytes32)
    {
        ...
        Client.Any2EVMMessage memory executableMsg = Client.Any2EVMMessage({
            messageId: mockMsgId,
            sourceChainSelector: s_currentChainSelector, // Sepolia
    ...
    function getOnRamp(uint64 /* destChainSelector */ ) public pure returns (address onRampAddress) {
